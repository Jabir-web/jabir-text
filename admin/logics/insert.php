...
if (isset($_POST['serviceaddbtn'])) {
    ...
    $query="INSERT INTO `expenses` (`expense_id`, `expense_name`, `expense_amount`, `expense_date`,`exp_admin`) VALUES (NULL, '{$ename}', '{$eamount}', current_timestamp(),'{$useradminid}');";
    ...
    $des = $servname;
    $sql="INSERT INTO `billing` (`bill_id`, `bill_cname`, `bill_cnumber`, `bill_staff`,`bill_month`, `bill_description`, `bill_amount`, `bill_discount`, `bill_date`,`bill_admin`) VALUES (NULL, '{$customername}', '{$customernumber}', '{$staff}','{$month}', '{$des}', '{$totalbill}', '{$discount}', current_timestamp(),'{$useradminid}')";
    if (mysqli_query($connection,$sql)) {
